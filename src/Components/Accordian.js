...
export default function QuestionTable() {
  ...
  const handleChangeTecchnology =(event)=>{
    ...
    // Call fetchData function passing selectedTechnology as a parameter
  ...
  const fetchData = async (companyId = null, technologyId = null) => {
    try {
      let response;
      if (companyId === null) {
        if (id === "id") {
          if (technologyId === null) {
            ...
            response = await axios.get(
              `http://127.0.0.1:8000/api/interview_tracking/question/?technology_id=${technologyId}`
            );
          }
        } else {
          if (technologyId === null) {
            ...
            response = await axios.get(
              `http://127.0.0.1:8000/api/interview_tracking/question/?company_id=${companyId}&technology_id=${technologyId}`
            );
          }
        }
      } else {
        if (technologyId === null) {
          response = await axios.get(
            `http://127.0.0.1:8000/api/interview_tracking/question/?company_id=${companyId}`
          );
        } else {
          response = await axios.get(
            `http://127.0.0.1:8000/api/interview_tracking/question/?company_id=${companyId}&technology_id=${technologyId}`
